...
const TIPOS = [
  ...
  { value: 'pedagogico', label: 'Pedagogico', color: 'bg-green-100 text-green-700' },
...
export default function ComunicadosView({ user }: { user?: any }) {
  ...
  const enviarComunicado = async () => {
    if (!form.titulo || !form.mensagem) { toast.error('Preencha titulo e mensagem!'); return; }
    ...
    if (destinatarios.length === 0) { toast.error('Nenhum responsavel com telefone cadastrado!'); return; }
    setSending(true);
    ...
    toast.success('Comunicado enviado para ' + destinatarios.length + ' responsavel(is)!');
    setShowModal(false);
    ...
    <div className="space-y-6 pb-12">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Envie avisos em massa para os responsaveis via WhatsApp</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Este Mes', value: comunicados.filter(c => c.created_at?.startsWith(new Date().toISOString().slice(0,7))).length, color: 'text-blue-600' },
          { label: 'Responsaveis', value: students.filter(s => s.parent_phone).length, color: 'text-green-600' },
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-gray-100 flex items-center justify-between">
          <h3 className="font-black text-gray-900">Historico de Comunicados</h3>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 mt-2 text-xs text-gray-400">
                        <span className="flex items-center gap-1"><Users size={11} /> {c.total_enviados} responsavel(is)</span>
                        <span className="flex items-center gap-1"><Clock size={11} /> {new Date(c.created_at).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                        {c.criado_por_nome && <span>por {c.criado_por_nome}</span>}
                      </div>
                    </div>
                    <span className="flex items-center gap-1 text-xs font-bold text-green-600 bg-green-50 px-3 py-1.5 rounded-lg shrink-0">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <textarea rows={4} value={form.mensagem} onChange={e => setForm(f => ({ ...f, mensagem: e.target.value }))}
                  placeholder="Digite a mensagem que sera enviada para os responsaveis..."
              ...
              <div>
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Destinatarios</label>
                    ...
                    {students.map(s => (
                      <label key={s.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl cursor-pointer hover:bg-purple-50 transition-all">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400">{s.parent_name || 'Sem responsavel'} {s.parent_phone ? '- ' + s.parent_phone : '- Sem telefone'}</p>
                        </div>
                      </label>
                    ))}
                  </div>
              ...
              <div className="bg-blue-50 rounded-xl p-3 text-xs text-blue-700 font-bold">
                O WhatsApp sera aberto para cada responsavel com a mensagem ja preenchida.
              </div>
            </div>
