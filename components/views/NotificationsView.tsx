...
export default function NotificationsView({ user }: { user?: any }) {
  ...
  const confirmReject = async () => {
    ...
    try {
      ...
      await supabase.from('schedules').update({ status: 'confirmado', attendance_status: null }).eq('id', rejectingNotif.schedule_id);
      const { data: teacherData } = await supabase.from('teachers').select('email').eq('id', scheduleData?.teacher_id).single();
      const { data: profProfile } = await supabase.from('profiles').select('id').eq('email', teacherData?.email || '').single();
      const notifUserId = profProfile?.id || scheduleData?.teacher_id;
      await supabase.from('notifications').insert({ user_id: notifUserId, title: 'Aula devolvida para correcao', message: 'O administrador devolveu a aula de ' + (scheduleData?.subject || 'Aula') + ' com ' + (scheduleData?.student_name || '') + '. Motivo: ' + rejectMotivo + '. Por favor, corrija e reenvie.', type: 'warning', read: false, created_at: new Date().toISOString() });
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md">
            <div className="p-6 border-b border-gray-100">
              <h2 className="text-lg font-black text-gray-900">Motivo da devolucao</h2>
              <p className="text-xs text-gray-400 mt-1">A professora sera notificada com este motivo</p>
            </div>
            <div className="p-6">
              <textarea rows={4} value={rejectMotivo} onChange={e => setRejectMotivo(e.target.value)} placeholder="Ex: O aluno compareceu. Por favor, marque como presente e reenvie." className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm resize-none focus:outline-none focus:ring-2 focus:ring-red-300" />
            </div>
            <div className="p-6 border-t border-gray-100 flex gap-3">
              <button onClick={() => setRejectingNotif(null)} className="flex-1 py-3 border border-gray-200 text-gray-600 rounded-xl text-sm font-bold hover:bg-gray-50 transition-all">Cancelar</button>
              <button onClick={confirmReject} disabled={savingReject || !rejectMotivo.trim()} className="flex-1 py-3 bg-red-500 hover:bg-red-600 disabled:opacity-50 text-white rounded-xl text-sm font-bold transition-all">{savingReject ? 'Enviando...' : 'Devolver para professora'}</button>
            </div>
          </div>
        </div>
      )}
